...
// clear && clang++ -std=c++17 -Wall step3.cpp && ./a.out && echo $?
...
//    for example, we need to support call paremeters like this : (obj, 1, 2) with param3 being defaulted
...
int main()
{
    ...
    iResult = checkIfPositive<int, OBJ_INT>(objInt, -1, -2, -3, total); //<-- Total is output param
    assert(total == -6);
    iResult = checkIfPositive<int, OBJ_INT>(objInt, 0, 0, 0, total); //<-- Total is output param
    assert(total == 0);
