...
// I tried to think of a way to do this elegantly with variadic template arguments, but realized that even if it was possible,
// it'd require some complex template work (index sequences and what not), and I can't touch the call above either, so I
// chose to go with an overload, which may not be as good-looking, but it'll allow me to get the total and resume the
// processing the previous method did, and any calls with less parameters will go straight to the previous method.
template <typename T, typename U>
