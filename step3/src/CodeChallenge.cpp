...
// Add template with additional 'total' parameter
// Note that param3 does not appear to be optional in these calls, so we can eliminate the '= 0' which would 
// need to appear as the last parameter.
template <typename T, typename U>
...
int main()
{
    ...
    std::cout << sResult.c_str() << std::endl; // Add c_str() to eliminate warnings (unless we really can't change anything in this section)
    ...
    */
    ...
    cout << endl << "New tests including 'total' parameter" << endl;
    ...
    sResult = checkIfPositive<std::string, OBJ_STR>(objStr, 1, 2, 3, total); //<-- Total is output param
    std::cout << sResult.c_str() << " " << total << std::endl;
