...
template <typename T, typename U, typename ...TotalArg>
T checkIfPositive(const U& u, const int& param1, const int& param2,
                  int param3 = 0, TotalArg&...total)
{
    static_assert(sizeof...(total) <= 1, "too many args for total");
    if constexpr (sizeof...(total) == 1) {
