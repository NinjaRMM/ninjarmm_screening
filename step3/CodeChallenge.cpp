...
{
    T value = checkIfPositive<T, U>(u, param1, param2, param3);
    total += value;
    return value;
}

template <>
std::string checkIfPositive(const OBJ_STR& u, const int& param1, const int& param2, int param3, int& total)
{
    std::string value = checkIfPositive<std::string, OBJ_STR>(u, param1, param2, param3);
    if(value == "true")
    {
        ++total;
    }
