...
// License: MIT License
// (https://github.com/torpedro/microtest.h/blob/master/LICENSE) Copyright (c)
// 2017 Pedro Flemming
//
...
public:
  AssertFailedException(std::string description, std::string filepath, int line)
      : std::exception(), description_(description), filepath_(filepath),
        line_(line){};
...
#define TEST_MAIN()                                                            \
  int main(int argc, char *argv[]) {                                           \
    ...
    if (num_failed == 0) {                                                     \
      ...
      double percentage =                                                      \
          100.0 * num_failed / mt::TestsManager::tests().size();               \
