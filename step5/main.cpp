...
// I have added the total in the template on line 31 and 40.
// But a reference int is needed at the line 31 rather than an int.
// So the total cannot be returned to the function caller.
// I don't think there is a solution in this question to have 'total'
...
int main()
{
    ...
    iResult = checkIfPositive<int, OBJ_INT>(objInt, 1, 2, 3, totalRef); //<-- Total is output param
    ...
    sResult = checkIfPositive<std::string, OBJ_STR>(objStr, 1, 2, 3, totalRef); //<-- Total is output param
    std::cout << sResult << " " << totalRef << std::endl;
