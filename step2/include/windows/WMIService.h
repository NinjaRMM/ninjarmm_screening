...
{
public:
	static WMIService& GetInstance()
	{
		// Lazy Instantiation on first use.
		static WMIService    instance; // Guaranteed to be destroyed.
		return instance;
	}
	...
	void Release();
	bstr_t BuildQueryString(const wchar_t* wmiClass, std::vector<LPCWSTR> attrs);
	IEnumWbemClassObject* ExecuteWQLQuery(const wchar_t* wmiClass, const std::vector<LPCWSTR>& attrs);
	IEnumWbemClassObject* ExecuteWQLQuery(BSTR stringQuery);

	WMIService(WMIService const&) = delete;
	void operator=(WMIService const&) = delete;
private:
	WMIService();
	~WMIService();
